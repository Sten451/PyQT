...
class ClientTransport(threading.Thread, QObject):
    ...
    def create_presence(self):
        ...
        logger.debug(f'Сформировано {CODE_PRESENCE} сообщение для пользователя {self.username}')
        return out
    ...
    def process_server_ans(self, message):
        ...
        if CODE_RESPONSE in message:
            if message[CODE_RESPONSE] == 200:
                ...
                logger.debug(f'Принят неизвестный код подтверждения {message[CODE_RESPONSE]}')
